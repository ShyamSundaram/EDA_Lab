...
Basic calculus requires that we find the 1st derivative, and solve for the value of x such that f'(x) = 0. 

This is easy enough to do, f'(x) = 2*1.2*(x-2)

Step factor/learning rate = 0.01, initial x value= 0.1 

Find the values of x that minimize the function above, and plot the progress of the algorithm with each iteration.
...
The actual solution we will approximate with gradient descent is  x = 2 as depicted in the plot below gradient descent implementation
```{r}
